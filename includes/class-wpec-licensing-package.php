<?php
class WPEC_Licensing_Package_Info {
	...
	public function __construct() {
		if ( ! function_exists('Markdown') ) {
			include_once( WPEC_LICENSING_PLUGIN_DIR . '/includes/markdown.php' ); //Used to convert readme.txt contents to HTML.
		}
	}
	 ...
	 * @return Wpup_Package
	 */
	public static function fromArchive($filename, $item_id, $slug = null ) {
		//Check if its cached first
		$cache_key = '_wpec_lic_product_readme_cache_' . $item_id;
		$readme = get_transient( $cache_key );

		// If the cache doesn't exist
		if( false === $readme ) {
			$metaObj = new WpecZipMetadataParser($slug, $filename);
			$readme = $metaObj->get();

			//Cache the readme data for a period of time.
			set_transient( $cache_key, $readme, HOUR_IN_SECONDS * 12 );
		}		
...
class WpecZipMetadataParser {
	 ...
	 */
	public function __construct($slug, $filename){
		$this->slug = $slug;
	 ...
	 *
	 * @throws Wpec_InvalidPackageException if the input file can't be parsed as a plugin or theme.
	 */
	protected function extractMetadata(){
		$this->packageInfo = WpecWordPressPackageParser::parsePackage($this->filename, true);
		if ( is_array($this->packageInfo) && $this->packageInfo !== array() ){
			...
			throw new Wpec_InvalidPackageException( sprintf('The specified file %s does not contain a valid WordPress plugin or theme.', $this->filename));
		}
	}
...
class WpecWordPressPackageParser {
	/**
...
class Wpec_InvalidPackageException extends RuntimeException { }